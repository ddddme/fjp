    ...
    public class WxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            Log.WriteLog("OrderQuery request : " + xml);
            var response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            Log.WriteLog("OrderQuery response : " + response);
	    ...
	    */
        public static WxPayData RefundQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            Log.WriteLog("RefundQuery request : " + xml);
            var url = "https://api.mch.weixin.qq.com/pay/refundquery";
            var response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口以提交数据到API
            Log.WriteLog("RefundQuery response : " + response);
        ...
        */
        public static WxPayData DownloadBill(WxPayData inputObj, int timeOut = 6)
        {
            ...
            Log.WriteLog("DownloadBill request : " + xml);
            var url = "https://api.mch.weixin.qq.com/pay/downloadbill";
            var response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口以提交数据到API
            Log.WriteLog("DownloadBill result : " + response);
        ...
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6)
        {
            ...
            if (!inputObj.IsSet("openid") &&
                inputObj.GetValue("trade_type").ToString().EqualsIgnoreCase("JSAPI"))
                throw new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");

            if (!inputObj.IsSet("product_id") &&
                inputObj.GetValue("trade_type").ToString().EqualsIgnoreCase("NATIVE"))
            ...
            var xml = inputObj.ToXml();
            var start = DateTime.Now;
            Log.WriteLog("UnfiedOrder request : " + xml);

            var url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            var response = HttpService.Post(xml, url, false, timeOut);
            Log.WriteLog("UnfiedOrder response : " + response);
	    ...
	    */
        private static void ReportCostTime(string interface_url, int timeCost, WxPayData inputObj)
        {
            ...
            if (WxPayConfig.REPORT_LEVENL == 1 &&
                ...
                inputObj.GetValue("return_code").ToString().EqualsIgnoreCase("SUCCESS") &&
                inputObj.GetValue("result_code").ToString().EqualsIgnoreCase("SUCCESS"))
                return;
            ...
            //返回信息
            else if (inputObj.IsSet("return_msg"))
                data.SetValue("return_msg", inputObj.GetValue("return_msg"));

            //业务结果
            else if (inputObj.IsSet("result_code"))
                data.SetValue("result_code", inputObj.GetValue("result_code"));

            //错误代码
            else if (inputObj.IsSet("err_code"))
                data.SetValue("err_code", inputObj.GetValue("err_code"));

            //错误代码描述
            else if (inputObj.IsSet("err_code_des"))
            ...
            //设备号
            else if (inputObj.IsSet("device_info"))
	    ...
	    */
        public static WxPayData Report(WxPayData inputObj, int timeOut = 1)
        {
            ...
            Log.WriteLog("Report request : " + xml);

            var url = "https://api.mch.weixin.qq.com/payitil/report";
            var response = HttpService.Post(xml, url, false, timeOut);

            Log.WriteLog("Report response : " + response);
