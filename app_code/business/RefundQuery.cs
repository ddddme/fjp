namespace FlowRecharge.Wechat
{
    public class RefundQuery
    {
        ...
        */
        ...
        {
            Log.WriteLog("RefundQuery is processing...");

            var data = new WxPayData();
            if (!string.IsNullOrWhiteSpace(refund_id))
                data.SetValue("refund_id", refund_id);//微信退款单号，优先级最高 
            else if (!string.IsNullOrWhiteSpace(out_refund_no))
                data.SetValue("out_refund_no", out_refund_no);//商户退款单号，优先级第二 
            else if (!string.IsNullOrWhiteSpace(transaction_id))
                data.SetValue("transaction_id", transaction_id);//微信订单号，优先级第三 
            else
                data.SetValue("out_trade_no", out_trade_no);//商户订单号，优先级最低 

            var result = WxPayApi.RefundQuery(data);//提交退款查询给API，接收返回数据

            Log.WriteLog("RefundQuery process complete, result : " + result.ToXml());
