...
namespace FlowRecharge.Wechat
{
    public class JsApiPay
    {
        ...
        */
        ...
        {
            if (!string.IsNullOrWhiteSpace(page.Request.QueryString["code"]))
            {
                //获取code码，以获取openid和access_token
                var code = page.Request.QueryString["code"];
               Log.WriteLog("获取code码: " + code);
                this.GetOpenidAndAccessTokenFromCode(code);
            }
            else
            {
                //构造网页授权获取code的URL
                var host = page.Request.Url.Host;
                var path = page.Request.Path;
                var redirect_uri = HttpUtility.UrlEncode("http://" + host + path);
	    ...
	    */
        public void GetOpenidAndAccessTokenFromCode(string code)
        {
            ...
            {
                Log.WriteLog("网页授权错误：" + ex.ToString());
        ...
        */
        ...
        {
            Log.WriteLog("JsApiPay::GetJsApiParam is processing...");
            ...
            Log.WriteLog("Get jsApiParam : " + parameters);
            return parameters;
        }
	    ...
	    */
        public string GetEditAddressParameters()
        {
            try
            {
                var host = page.Request.Url.Host;
                var path = page.Request.Path;
                var queryString = page.Request.Url.Query;
                //这个地方要注意，参与签名的是网页授权获取用户信息时微信后台回传的完整url
                var url = "http://" + host + path + queryString;

                //构造需要用SHA1算法加密的数据
                var signData = new WxPayData();
                ...
                var param = signData.ToUrl();
                Log.WriteLog("SHA1 encrypt param : " + param);

                //SHA1加密
                var addrSign = FormsAuthentication.HashPasswordForStoringInConfigFile(param, "SHA1");
                Log.WriteLog("SHA1 encrypt result : " + addrSign);
                ...
                //转为json格式
                var parameter = afterData.ToJson();
                Log.WriteLog("Get EditAddressParam : " + parameter);
